...
- English and Russian app interface, selected automatically from the phone language.
...
2. Upload the project contents. Do not upload `local.properties`, `.gradle/`, `build/`, or the SDK AAR.
3. In Android Studio, build `app-debug.apk` or a signed release APK.
4. Open the repository's **Releases** page, create a release tag such as `v1.3`, and attach the APK.
...
The ready-to-test APK is included at [`release/glyph-composite-toy-v1.3-debug.apk`](release/glyph-composite-toy-v1.3-debug.apk). It is a debug build for personal installation and Glyph Toy testing. A signed release APK should be used for Play Store publication.
